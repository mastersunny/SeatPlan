            ...
            <li>
              ...
              <div class="collection">
                  @foreach($exam->exams as $exam_type)
                    <a href="" data-year="{{$exam->year}}" data-session="{{$exam->session}}" data-examid="{{$exam_type->id}}" data-batch="{{$student->batch_id}}" class="exam collection-item">{{$exam_type->exam_type->name}}</a>
                  @endforeach
              </div>
              </div>
            </li>
        ...
        <div class="row">
          ...
          <div class="card">
            <div class="col s12 m4" style="padding:3%;">  
              <img class="rounded" src="{{asset($student->url)}}">
            </div>
            
            <div class="col s12 m8" style="padding:3%;">
              <h5>Name: <i>{{$student->first_name}} {{$student->last_name}}</i></h5>
              <h5>Email: <i>{{$student->email_no}}</i></h5>
              <h5>Contact: <i>{{$student->contact_no}}</i></h5>
            </div>
  ...
  $(document).ready(function(){
    ...
    $('.exam').on('click',function(e){
      ...
      $('tbody#examRoutine').empty();
       ...
       var url = ((baseUrl)+'examRoutine/'+exam_id+"/"+batch_id);
       ...
       if(response[0].exam_routines.length==0){
        ...
        for(var i =0;i<response[0].exam_routines.length;i++){
           ...
           // session = response[0].session;
            ...
            semester_id = response[0].semester_id;
           ...
           for(var j=0;j<response[0].exam_routines[i].room_assigned.length;j++){
                ...
                $button  = $('<a href="" data-routineid='+exam_routine_id+' data-course='+course_id+' data-semester='+semester_id+' class="btn waves-effect waves-light">Seat</a>');
                ...
                $('tbody#examRoutine').append("<tr>"
                        ...
                        $("td#"+response[0].exam_routines[i].room_assigned[j].room_assign_id).append($button).click(function(e){
                             ...
                             semester_id = $(this).children('a').data('semester');
                            ...
                            attendence(course_id,semester_id,exam_routine_id);
   ...
   function attendence(course_id,semester_id){
             

             $('#modal1').openModal();

             url = baseUrl+'registeredStudents/'+course_id+'/'+semester_id;
